...
Includes the
link:{{< relref "api/" >}}#project__branches__branchUuid__schemas_get[schema migration status]
and
link:{{< relref "api/" >}}#project__branches__branchUuid__microschemas_get[mircoschema migration status]
for every branch of every project.

=== Plugins
Query name: `plugins`

Includes the link:{{< relref "api/" >}}#admin_plugins_get[currently deployed plugins].

=== Status
Query name: `status`

Includes the
link:{{< relref "api/" >}}#_get[version],
link:{{< relref "api/" >}}#admin_cluster_status_get[cluster status]
and
link:{{< relref "api/" >}}#search_status_get[elastic search status].
...
* Using the link:{{< relref "api/" >}}#admin_config_post[REST API]. Changing read only mode in this way will take immediate effect.
