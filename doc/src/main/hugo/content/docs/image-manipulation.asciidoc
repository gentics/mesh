...
Endpoint: ```{apiLatest}/:projectName/webroot/path/to/an/image?h=300```]
...
```GET {apiLatest}/:projectName/webroot/path/to/an/image?w=500&h=200```

.When adding the resize parameter `prop` the image gets scaled proportionally to fit the dimensions.
```GET {apiLatest}/:projectName/webroot/path/to/an/image?w=500&h=200&resize=prop```

.When adding the resize parameter `force` the image gets distorted to fit the dimensions
```GET {apiLatest}/:projectName/webroot/path/to/an/image?w=500&h=200&resize=force```
...
----
POST {apiLatest}/:projectName/nodes/df8beb3922c94ea28beb3922c94ea2f6
{
  "language": "en",
	"version": "draft",
  "fields": {
...
[source,js,subs="+attributes"]
----
POST {apiLatest}/:projectName/nodes/df8beb3922c94ea28beb3922c94ea2f6/binaryTransform/image
{
  "version" : "1.0",
  "language" : "en",
  "width" : 300,
