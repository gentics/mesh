...
API tokens can be generated via the `POST {apiLatest}/users/:userUuid/token` endpoint.
These tokens should not be publicly exposed since they they are equivalent to user credentials.
Tokens can be removed via the `DELETE {apiLatest}/users/:userUuid/token/:tokenUuid` endpoint.
