...
| ```tx.retry.delay```                            | Number  | 6000           | Sets the initial transaction retry delay which is applied when retrying failed transactions due to concurrent changes. A value of 0 will disable the delay.
|======
