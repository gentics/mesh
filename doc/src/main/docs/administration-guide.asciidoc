...
The total amount of required memory is the roughly sum of `heap memory` + `direct memory` + (`stack size` * `thread count`) + `64 MB (JVM base memory)`.
...
| 1_000 to 10_000      | `-Xms786m -Xmx786m -Xss2m -XX:MaxDirectMemorySize=384m  -Dstorage.diskCache.bufferSize=384`
| 10_000 to 100_000    | `-Xms1250m -Xmx1250m -Xss2m -XX:MaxDirectMemorySize=512m  -Dstorage.diskCache.bufferSize=512`
| 100_000 to 1_000_000 | `-Xms2500m -Xmx2500m -Xss4m -XX:MaxDirectMemorySize=1024m -Dstorage.diskCache.bufferSize=1024`
|======
