...
| 0 to 100             | `-Xms128m  -Xmx128m  -XX:MaxDirectMemorySize=128m`
| 100 to 1_000         | `-Xms512m  -Xmx512m  -XX:MaxDirectMemorySize=256m`
| 1_000 to 10_000      | `-Xms786m  -Xmx786m  -XX:MaxDirectMemorySize=384m`
| 10_000 to 100_000    | `-Xms1250m -Xmx1250m -XX:MaxDirectMemorySize=512m`
| 100_000 to 1_000_000 | `-Xms2500m -Xmx2500m -XX:MaxDirectMemorySize=1024m`
|======
...
The ```POST /api/v1/admin/graphdb/restore``` endpoint can be used to restore created backups. 
...
The `POST /api/v1/admin/graphdb/export` and `POST /api/v1/admin/graphdb/import` endpoints can be used to generate Graph Database export files which are gzipped json files. A large database can be exported and reimported again to reduce the sparse file size.
