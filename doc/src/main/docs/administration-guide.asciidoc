...
| `-resetAdminPassword`  | String  | Set the admin password which will be used to override the existing admin user password. It is advised to change the password once again after the reset has been performed.
| `-httpPort`            | Number  | Override the used http port from the `mesh.yml` file.
| `-clusterName`         | String  | Override the configured cluster name from the `mesh.yml` file.
| `-nodeName`            | String  | Override the configured node name from the `mesh.yml` file.
| `-initCluster`         | -       | Argument that can be used to configure Gentics Mesh in a way so that the Datebases are directly ready to be used in a clustering environment. This is usually only used for testing or setup of fresh cluster instances.
|======
