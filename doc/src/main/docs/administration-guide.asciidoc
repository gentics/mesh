...
|======
| Configuration              | Type     | Default          | Description
| ```search.directory```     | Path     | data/searchindex | Path to the elasticsearch storage location.
| ```search.httpEnabled```   | Boolean  | false            | Enable the elasticsearch http endpoint. Enabling this has security implications! See security guide for more information.
| ```search.transportPort``` | String   | 9300-9400        | Range for the binary transport ports
|======
...
Gentics Mesh does not manage any cache structure but it is possible to tweak the underlying OrientDB and Elasticsearch database cache settings.
