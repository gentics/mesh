...
| ```tx.retry.delay```                            | Number  | 0              | Sets the initial transaction retry delay in milliseconds which is applied when retrying failed transactions due to concurrent changes. A value of 0 will disable the delay.
|======
