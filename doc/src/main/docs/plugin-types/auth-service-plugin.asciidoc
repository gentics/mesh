...
The information from the returned `MappingResult` will be used to sync the elements in Gentics Mesh. Any of the elements can be omitted.
...
6. Iterate over all roles of the mapped groups and invoke `AuthServicePlugin#removeRoleFromGroup` to check whether the role should be unlinked from the group. You can use this to ensure that only specific roles are linkes to the groups.
7. Iterate over all groups of the user and invoke `AuthServicePlugin#removeUserFromGroup` to check whether the user should be removed from the group. You can use this to ensure that the use is only part of specific groups.
