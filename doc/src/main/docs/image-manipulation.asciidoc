...
== Fetch and Resize an Image with WebRoot Path
...
It also possible to provide both dimensions. Note that in this case Mesh potentially crops your image to fit the desired dimensions. This happens when the aspect ratio of the source image is different from the requested dimensions.
Should you _not_ want to crop the image but instead have Mesh transform the image to fit the requested width and height you can use the resizing option `resize=force`.
Note that using the force-resizing mode can potentially lead to non-proportional resizing and thus potentially distort your image. Since this is seldomly desired the default resizing option, that gets used when no resize-parameter is provided, is `resize=smart`.

.When providing width and height Mesh crops the image to the desired format 
...
== Fetch and Crop an Image with WebRoot Pat
