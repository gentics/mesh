...
The index sync is a memory intensive operation. The `search.syncBatchSize` setting can be used to control how much memory will be utilized during the synchronization. With 250 MB of the Heap memory is sufficient to index `250_000` elements.
