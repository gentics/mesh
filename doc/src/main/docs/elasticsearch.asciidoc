...
It is also possible to define different settings / mappings for different languages. To do so, add the `_meshLanguageOverride` property to the `elasticsearch` object.
...
For each overridden language, Gentics Mesh will create additional indices in Elasticsearch. Node contents of that language will be saved in that index.
