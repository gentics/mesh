...
This layer will automatically delgate requests to an elected master instance.
...
In an master-replica setup this can be useful to delegate write requests on replica instances to the elected master instance.
...
The `cluster.coordinatorTopology` setting or `MESH_CLUSTER_COORDINATOR_TOPOLOGY_ENV` environment variable controls whether the coordinator will also manage the current cluster topology. By default no cluster topology management will be done.
