...
There are two types of upgrades. An upgrade can either support live upgrade or offline upgrade. 

=== Live Upgrade

Live upgrades can happen without any downtime. First you stop a single instance within your cluster. Next you start the instance up again using the new Gentics Mesh version.
...
order to determine whether an live or offline upgrade can be done.
