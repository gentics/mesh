...
* `MODE_A` - In this mode only write requests will be delegated to the elected master. All other requests will be directly processed by the instance.

* `MODE_B` - In this mode all requests (read and write) will be delegated to the elected master.
