...
CAUTION: This section is irrelevant, if link:/premium-features/sql-db/#docs[SQL RDBMS storage premium feature] is used. Please refer to the feature documentation, as well as the clustering documentation for the chosen RDBMS.
...
The replication handles this as fast as the network allows it. Some minor lag, utilizing clearing the caches, is possible.
