...
The list of public keys which will be used to validate and accept JWT's can be added to the `mesh.yml` file in the `security.publicKeys` list.
...
The `AuthServicePlugin` API provides methods which can be used to supply JWK's to Gentics Mesh.
