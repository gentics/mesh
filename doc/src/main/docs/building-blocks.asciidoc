...
| resolveLinks
| string
| The ```resolveLinks``` parameter can be set to either ```short```, ```medium``` or ```full```. Links will automatically be resolved and replaced by the resolved http://getmesh.io/docs/beta/#_webroot[WebRoot] link. No resolving occurs if the query parameter is omitted.
...
TIP: Using the ```container``` property, a schema can be configured to allow for hierarchically structuring nodes. Nodes based on a such a schema may contain child nodes. This is the basis for building http://getmesh.io/docs/beta/#_contenttrees[content trees] in Gentics Mesh and leveraging the power of http://getmesh.io/docs/beta/#_navigation[automatic navigation menus, breadcrumbs and link resolving].
