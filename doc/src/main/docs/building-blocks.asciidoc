...
Setting the extract object will override the global configurations for parsing ({{< relref "administration-guide.asciidoc" >}}#_upload_options[```upload.parser```]) and indexing (link:{{< relref "administration-guide.asciidoc" >}}#_search_options[```search.includeBinaryFields```]) uploads for this field.

Setting the global metadata whitelist ({{< relref "administration-guide.asciidoc" >}}#_upload_options[```upload.metadataWhitelist```]) will filter the parsed metadata accordingly.
