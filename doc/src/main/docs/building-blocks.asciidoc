...
This part of our documentation will be about the core concepts of Gentics Mesh. It will describe the building blocks you'll need to setup your project. We will start with an overview and then move on to discuss each concept in detail including examples and references to relevant parts of the API. At the end you should be able to understand the core concepts and be ready to design your content model as well as manage users and their permissions on elements of your project using either the API or the Gentics Mesh user interface.
...
TIP: Using the ```container``` property, a schema can be configured to allow for hierarchically structuring nodes. Nodes based on a such a schema may contain child nodes. This is the basis for building http://getmesh.io/docs/beta/#_contenttrees[content trees] in Gentics Mesh and leveraging the power of automatic http://getmesh.io/docs/beta/#_navigation[navigation menus], http://getmesh.io/docs/beta/#_breadcrumbs[breadcrumbs] and http://getmesh.io/docs/beta/#_prettyurls[pretty URLs].
...
| Apply the provided changes on the latest version of the schema and http://getmesh.io/docs/beta/#_schema_microschema_migration[migrate] all nodes which are based on the schema.
| ```POST /api/v1/schema/:schemaUuid/changes```

| Compare the given schema with the stored schema and create a changeset for http://getmesh.io/docs/beta/#_schema_microschema_migration[migration]. 
...
| Apply the provided changes on the latest version of the microschema and http://getmesh.io/docs/beta/#_schema_microschema_migration[migrate] all micronodes which are based on the schema.
| ```POST /api/v1/microschemas/:microschemaUuid/changes```

| Compare the provided schema with the schema which is currently stored and generate a set of changes that have been detected for http://getmesh.io/docs/beta/#_schema_microschema_migration[migration].
...
| resolveLinks
| string
| The ```resolveLinks``` parameter can be set to either ```short```, ```medium``` or ```full```. With the parameter set the ```path``` property as well as the ```languagesPath``` property (for available language variants) will be included in the response. Gentics Mesh links in any HTML-typed field will automatically be resolved and replaced by the resolved hhttp://getmesh.io/docs/beta/#_link_resolving[WebRoot] link. No resolving occurs if the query parameter is omitted.
