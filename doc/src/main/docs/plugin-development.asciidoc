...
| `PluginContext#client()`      | Return a new client which can be used to access the API . This client will in contrast to `MeshPlugin#adminClient()` use the 
|======
...
| `GraphQLPluginContextprojectUuid()`   | Uuid of current project.
...
The configration of the plugin will be stored in the plugin directory in a `plugins/{pluginId}/config.yml` file which can be overriden by a `/plugins/{pluginId}/config.local.yml` file.
