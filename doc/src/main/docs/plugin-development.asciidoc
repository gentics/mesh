...
The plugin needs at least this dependency. It is important to set the scope to `provided`. Otherwise the plugin can't be loaded by Gentics Mesh.
...
public static final MeshLocalServer server = new MeshLocalServer()
    .withInMemoryMode()
    .withPlugin(MyPlugin.class, "myPlugin")
    .waitForStartup();
```
