...
Add the interface `GraphQLPlugin` to your plugin class.
...
* **Starting** - Plugin gets started. (`MeshPlugin#start()` will be invoked)
* **Initialisation** - Plugin gets initialized. (`MeshPlugin#initialize()`)
* **Registration** - Plugin gets finally registered so that it can be used.

=== Undeploy

* **De-registration** - Plugin registeration gets removed. At this point the plugin integration with the server gets removed. REST and GraphQL extensions get removed.
* **De-initialisation** - Plugin gets de-initialized.
* **Stopping** - Plugin gets stopped.  (`MeshPlugin#stop()`)
