...
For the control of a *response* body minification one needs a `DisplayParameters#setMinify(boolean)` set to the desired request. This parameter override the server-side JSON minification setup per each request.
