...
| String
| ```string```
| A string field type does not have any special configuration settings.
...
Each endpoint will return a navigation response which contains the nested navigation tree structure. The ```maxDepth``` parameter may be used to limit the navigation depth. By default a ```maxDepth``` value is of `10` is used.
...
[source,json]
----
include::{es-json}/microschema.search.json[]
----
