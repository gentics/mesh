...
In order to be able to store and retrieve content, a Gentics Mesh user needs to authenticate (`username:password`). Gentics Mesh provides multiple ways of authentication:

* using basic authentication header
* posting JSON Data
* sending a valid JSON Web token (JWT)
...
===== JWT Configuration
The JWT auth provider requires a keystore for signing and verifying the generated tokens. 
...
Alternatively, you can use the https://docs.oracle.com/javase/8/docs/technotes/tools/windows/keytool.html[Java keytool] to create a new keystore. Here is an example on how to create a keystore:
[source,bash]
----
