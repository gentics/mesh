...
| ```/api/v1/:projectName/nodes/:uuid``` | Read Published | _Not used_
| ```/api/v1/:projectName/nodes/:uuid``` | Publish        | _Not used_
|======
...
|======
| Element                              | Permission     | Description
| ```/api/v1/:projectName/releases```  | Create         | _Not used_
...
|======
| Element                                    | Permission     | Description
| ```/api/v1/:projectName/releases/:uuid```  | Create         | _Not used_
| ```/api/v1/:projectName/releases/:uuid```  | Read           | _Not used_
| ```/api/v1/:projectName/releases/:uuid```  | Update         | _Not used_
| ```/api/v1/:projectName/releases/:uuid```  | Delete         | _Not used_
...
It is possible to load older version by specifying the ```?version``` parameter. Similarly the published version of can also be loaded using the parameter ```?version=published``` or the draft version via ```?version=draft```.
...
NOTE: It is current not possible to migrate from one release to another. There is currently no way to invoke an automatic merge of nodes from one specific release to another.
...
All operations to nodes can be scoped to a specific release by adding the ```?release=name``` query parameter. If omitted the latest release release will be used to execute the operation. 
...
A releases can be _renamed_ and new schema/microschema versions can be _assigned_ to it.
...
The node will be completely removed from the system if node more language variants are referenced within any release.
