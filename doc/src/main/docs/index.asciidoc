...
While it’s possible to organize your contents in a simple, flat structure with tagging and referencing, content trees come with a bonus: they inherently allow for automatically generated hierarchical <<navigation, navigation menus, breadcrumbs and pretty URLs>>. This furthermore means routes are not hard-coded in your app but can be defined dynamically by content editors within the Gentics Mesh user interface. 
...
Let’s consider the following example on the basis of our demo app to demonstrate how these properties work together to generate pretty URLs, breadcrumbs, and navigation menus:
...
The schema ```category``` has the ```container``` property set to ```true```, thus allowing nodes of type category to have child nodes. The schema’s ```segmentField``` is specified to be the ```name``` field.
The schema ‘vehicle’ has the ```container``` property set to ```false```. The schema’s ```segmentField``` is again specified to be the ```name``` field.
...
Instead of relying on UUIDs to link your content, you can use pretty URLs like http://yourapp.com/Automobiles/Trabant. For each node, Gentics Mesh will provide you with a human readable path. Likewise, you can query your content using WebRoot API instead of using UUIDs.
The ```segmentField``` is used to build the path of a specific node. According to the nodes’ schemas in our example, the path ```/Automobiles/Trabant``` for the node _Trabant_ is built using the ```name``` field of the ```vehicle``` node, i.e. _Trabant_, and the ```name``` field of the ```category``` node, i.e. _Automobiles_.

image:GenticsMesh-PrettyURL-Example.png[]

The ```path``` property as well as the ```languagesPath``` property (for available language variants) of a node can be queried using the ```/api/v1/{project}/nodes/{nodeUuid}``` endpoint together with the query parameter ```resolveLinks```. For more details see the documentation on http://getmesh.io/docs/beta/building-blocks.html#_node[node].
...
The API end point ```/api/v1/{project}/nodes/{nodeUuid}/navigation``` returns a navigation object for the provided node, while the ```/api/v1/{project}/navroot/{path}``` API end point returns a navigation object for a node which is located using a given path.
Each endpoint will return a navigation response which contains the nested navigation tree structure. The ```maxDepth``` parameter may be used to limit the navigation depth.
...
| maxDepth
| number (default: 10)
| Specifies the maximum depth for the request navigation tree structure.
...
Each node in Gentics Mesh provides information on where it is located within the node tree in terms of its ```breadcrumb``` property using the ```/api/v1/{project}/nodes/{nodeUuid}``` end point together with the query parameter ```resolveLinks```. 
...
Again, this functionality builds on top of content trees in Gentics Mesh. To be more specifically, the ```segmentField``` is used to build the path of a specific node (see also <<prettyurls, Pretty URLs>> for a detailed explanation).
...
A webroot path consists of multiple path segments, each corresponding to a node within the content tree. The ```segmentField``` property from the schema is used to determine the node's relevant field value. 

NOTE: Each segment of the URL must be URL-encoded. A node's language variants must provide differents path segment in order to guarantee unique paths.
...
The WebRoot API, furthermore, directly supports binary content using the ```fileName``` meta-data field of the ```binary``` field . 

*Fetch an image with UUID:*
http://getmesh.io/api/v1/demo/nodes/b1a975d9391a4daaa975d9391abdaa4f/binary/image

Downloading the binary field with the given name using API end point ```GET /api/v1/:projectName/nodes/:nodeUuid/binary/:fieldName```.
