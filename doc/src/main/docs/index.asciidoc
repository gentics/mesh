...
An API token can be used to authenticate a user. The API token will never expire. This is different from regular tokens which will be issued when calling `/api/v1/auth/login`. 
...
Typical usecases are backend implementations which constantly communicate with Gentics Mesh via a secure or local connection.

The token can be issued per user via `POST /api/v1/users/:userUuid/token`.
...
It is possible to manually revoke the issued token via `DELETE /api/v1/users/:userUuid/token`

==== Anonymous Access 

Sometimes it may be desirable to serve public content via Gentics Mesh. All Gentics Mesh instances ship with a included `anonymous` user/role/group. Requests to mesh which are not authenticated will automatically use the user with the username `anonymous` to authenticate and check the needed permissions.

You can assign `read` permissions to the `anonymous` role for all elements you want to be public.

NOTE: Assigning other permissions would of course allow for other operations to be granted. 

It is of course possible to disable this feature by deleting the `anonymous` and disabling the `enableAnonymousAccess` flag in the `security` section of the `mesh.yml` file.

NOTE: Recreating the `anonymous` user would automatically re-enable the feature if the configuration setting `enableAnonymousAccess` is set to `true`.
