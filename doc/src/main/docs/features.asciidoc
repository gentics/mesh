...
You can also disable this feature per field. See link:{{< relref "building-blocks.asciidoc" >}}#_schema_field_types[schema field types documentation] for more information.

You can set a whitelist of allowed metadata (({{< relref "administration-guide.asciidoc" >}}#_upload_options[```upload.metadataWhitelist```])). Setting that list will only save metadata with keys specified in that list.
