...
The ```path``` property as well as the ```languagesPath``` property (for available contents) of a node can be queried using the ```/api/v1/projectName/nodes/:nodeUuid``` endpoint together with the query parameter ```resolveLinks```. Alternatively, the WebRoot API can be used ```/api/v1/:projectName/webroot/:path```
...
.Try the example
[TIP]
https://demo.getmesh.io/api/v1/demo/nodes/688f7beae8d240bf8f7beae8d2a0bf2e/navigation?resolveLinks=short&maxDepth=1
...
| Load the node or the node's binary data, which is located using the provided path.
| link:++/docs/api#project__webroot__path__get[```GET /api/v1/:projectName/webroot/:path```]
...
You can specifiy the `hostname` and `ssl` flag when creating a project. This will in turn store this information within the created branch of the project. Gentics Mesh will make use of this information once you render a foreign node's link using the *short* type.
...
.Try the example
[TIP]
https://demo.getmesh.io/api/v1/demo/webroot/images/ford-gt.jpg?fpz=1.2&w=800&h=600&crop=fp&fpdebug=true
...
| link:/docs/api/#roles__roleUuid__permissions__pathToElement__get[```GET /api/v1/roles/:roleUuid/permissions/:path```]

| Set the permissions between role and the targeted element(s).
| link:/docs/api#roles__roleUuid__permissions__pathToElement__post[```POST /api/v1/roles/:roleUuid/permissions/:path```]
...
The link:/docs/api/#roles__roleUuid__permissions__pathToElement__get[```GET /api/v1/roles/:roleUuid/permissions/:path```] endpoint can be used to query a role's permission on a specific element or set of elements.
...
Alternatively, you can use query the element using the respective API endpoint (e.g. link:++/docs/api#project__nodes__get++[```GET /api/v1/projectName/nodes```]) and use the ```role``` query parameter to find out another role's (i.e. ```anonymous```) permissions on the element:
...
The endpoint link:/docs/api/#users__userUuid__permissions__path__get[```GET /api/v1/users/:userUuid/permissions/:path```] provides a shortcut for querying permissions for specific users directly, e.g., for user ```anonymous```, which is essentially inheriting all permissions from role ```anonymous```.
...
The link:/docs/api#roles__roleUuid__permissions__pathToElement__post[```POST /api/v1/roles/:roleUuid/permissions/:path```] API endpoint can be used to assign or revoke permissions to/from a role. 
...
Permissions on a single or multiple branches are applied via ```/api/v1/:projectName/branches/:uuid``` path and ```/api/v1/:projectName/branches``` path respectively.
...
|======
| Role permission                     | Element path                               | Permission     | Recursive
| Can create new branch               | ```/api/v1/:projectName/branches```        | Create         | false 
| Can read all branches               | ```/api/v1/:projectName/branches```        | Read           | true 
| Can update all branches             | ```/api/v1/:projectName/branches```        | Update         | true 
| Can read the branch                 | ```/api/v1/:projectName/branches/:uuid```  | Read           | false 
| Can update the branch               | ```/api/v1/:projectName/branches/:uuid```  | Update         | false 
|======
...
Publishing requires a ```POST``` request on the ```/api/v1/:projectName/nodes/:uuid/published``` endpoint, which will publish all contents of the node.
It is also possible to just publish specific contents with dedicated ```POST``` requests on ```/api/v1/:projectName/nodes/:uuid/languages/:languageTag/published```.
...
[source,json]
----
include::content/docs/api/response/api/v1/\{project\}/nodes/\{nodeUuid\}/published/200/example.json[]
----
...
Similarly to publishing, the contents of a node can be taken offline via a ```DELETE``` request on ```/api/v1/:projectName/nodes/:uuid/published```.
...
Versions of a node can be listed via the ```/api/v1/:projectName/nodes/:uuid/versions``` endpoint.
...
A version can be used as a `draft` version. This version can also be referenced as a `published` version when published. The `branchRoot` flag indicates whether the version is being referenced as root version in a branch. All these three different kind of versions are not purgeable. Versions which have been made in betweet can be purged.
...
Versioning can be controlled on a *per-schema* basis. The `versioned` property within the schema controls whether additional versions for a content will be kept.

The default versioning flag from the `mesh.yml` configuration file will be used if for schemas which don't have `versioned` property set. Setting the `versioned` property will thus also allow to override the default configuration in the `mesh.yml`.

The default behaviour can be configured via the environment variable `MESH_CONTENT_VERSIONING_ENABLED` 
or the config file.
...
NOTE: Disabling versioning will not stop version numbers to increase. Instead the mechanism will automatically purge the previously used version (if possible).

=== Purging versions

The ```POST /api/v1/projects/:projectUuid/maintenance/purge` endpoint can be used to purge older versions from the system.

When invoked a new job will triggered which purges all possible content versions from the given project.

The `?since` query parameter can be used to control what versions should be purged. 

The call `POST /api/v1/projects/:projectUuid/maintenance/purge?since=2019-04-20T18:00Z` will thus only purge versions which are older than the given date. Omitting the parameter will purge all versions.
...
Assigning schema versions to the branch via a ```PUT``` request to ```/api/v1/:projectName/branches/:uuid/schemas``` will automatically trigger a schema migration of the affected nodes contents within the branch.

[source,json]
----
include::content/docs/api/request/api/v1/\{project\}/branches/\{branchUuid\}/schemas/request-body.json[]
----

It is also possible to assign microschemas via a ```PUT``` request to ```/api/v1/:projectName/branches/:uuid/microschemas``` to the branch.

[source,json]
----
include::content/docs/api/request/api/v1/\{project\}/branches/\{branchUuid\}/microschemas/request-body.json[]
----
...
Deleting nodes is also branch specific. Deleting a node via ```DELETE``` to ```/api/v1/:projectName/node/:uuid?branch=winter2016``` will delete the node and all its sub nodes from the _winter2016_ branch.
The node will be completely removed from the system if no more contents are referenced within any branch.

It is also possible to just delete a specific language variant via ```DELETE``` to ```/api/v1/:projectName/node/:uuid/languages/en?branch=winter2016``` from branch _winter2016_.
...
[source,json]
----
include::content/docs/api/request/api/v1/schemas/\{schemaUuid\}/changes/request-body.json[]
----
