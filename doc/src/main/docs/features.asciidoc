...
Responsive design often suggests the usage of https://developer.mozilla.org/en-US/docs/Learn/HTML/Multimedia_and_embedding/Responsive_images[srcset images for responsive images].
Sometimes responsive designs require image sizes which lead to cropped images. This can lead to images which seem chopped off at the worst possible place or images which miss the important part.
