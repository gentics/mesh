...
While it is perfectly legitimate to organize your contents in a simple, flat structure with tagging and referencing, content trees come with big a bonus: they inherently allow for automatically generated hierarchical <<navigation, navigation menus>>, <<breadcrumbs,breadcrumbs>> and <<prettyurls,pretty URLs>>. This furthermore means routes are not hard-coded in your app but can be defined dynamically by content editors using the Gentics Mesh user interface. 
...
//== Automatic Navigation Menus, Breadcrumbs and pretty URLs
//If you choose to organize your content in terms of a content tree, Gentics Mesh allows you to automatically build navigation menus, breadcrumbs and pretty URLs. 
...
Within the demo project vehicles are organized in three different categories. The nodes _Automobiles_, _Aircrafts_ and _Yachts_ of type ```category``` serve as containers for nodes of type ```vehicle```, e.g., the node _Trabant_ in category _Automobiles_.

The schema ```category``` has the ```container``` property set to ```true```, thus allowing nodes of type `category` to have child nodes. The schema’s ```segmentField``` is specified to be the ```name``` field. Thus, the resulting value for the `path` property of node _Automobiles_ is `/Automobiles`.

The schema ‘vehicle’ has the ```container``` property set to ```false```. The schema’s ```segmentField``` is again specified to be the ```name``` field. Thus, the resulting value for the `path` property of node _Trabant_ is `/Automobiles/Trabant`.
...
The ```segmentField``` is used to build the path of a specific node. According to the nodes’ schemas in our <<example,example>>, the path ```/Automobiles/Trabant``` for the node _Trabant_ is built using the ```name``` field of the ```vehicle``` node, i.e. _Trabant_, and the ```name``` field of the ```category``` node, i.e. _Automobiles_.
...
| resolveLinks
| string
| The ```resolveLinks``` parameter can be set to either ```short```, ```medium``` or ```full```. Links will automatically be resolved and replaced by the resolved http://getmesh.io/docs/beta/#_link_resolving[WebRoot] link. No resolving occurs if the query parameter is omitted.
...
The ```breadcrumb``` property provides an array of node references representing the path from the current node up to the project root. For more details see the documentation on http://getmesh.io/docs/beta/building-blocks.html#_node[node].
...
image:GenticsMesh-LinkResolving-Example.png[]
//TODO image: For example, ```{{mesh.link("2f2de9297c8143e8ade9297c8193e8fc", "en")}}``` will be transformed into ```/api/v1/demo/webroot/Vehicle%20Images/ford-gt.jpg``` when using the `?resolveLinks=full`.
...
* `full` - the resolved links will contain the webroot prefix and project name.
* `medium` - the resolved links will contain the project name.
* `short` - will contain neither webroot prefix nor project name.
