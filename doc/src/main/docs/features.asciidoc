...
The ```path``` property as well as the ```languagesPath``` property (for available language variants) of a node can be queried using the ```{apiLatest}/projectName/nodes/:nodeUuid``` endpoint together with the query parameter ```resolveLinks```. Alternatively, the WebRoot API can be used ```{apiLatest}/:projectName/webroot/:path```
...
Publishing requires a ```POST``` request on the ```{apiLatest}/:projectName/nodes/:uuid/published``` endpoint, which will publish all language variants of the node.
It is also possible to just publish specific language variants with dedicated ```POST``` requests on ```{apiLatest}/:projectName/nodes/:uuid/languages/:languageTag/published```.
...
Similarly to publishing, the language variants of a node can be taken offline via a ```DELETE``` request on ```{apiLatest}/:projectName/nodes/:uuid/published```.
...
Assigning schema versions to the branch via a ```PUT``` request to ```{apiLatest}/:projectName/branches/:uuid/schemas``` will automatically trigger a schema migration of the affected nodes language variants within the branch.
...
The ```{apiLatest}/eventbus``` endpoint allows clients to access the eventbus. This may be useful if you want to react on specific events.
