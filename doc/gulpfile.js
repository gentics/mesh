...
// website url in production mode (must be absolute; protocol and host may be
// omitted)
var siteUrl = "" + contextPath;

// website url in development mode (must be absolute; protocol and host may be
// omitted)
...
function build(done, dev) {
	...
	Metalsmith(__dirname).source(paths.src).destination(paths.site)
...
gulp
		.task(
				...
				function(done) {
					...
					return request(
							"https://github.com/nerk/asciidoctor-bs-themes/archive/92dd167181d3c4ea48061ad4cba40ebc99d7151a.tar.gz")
...
gulp.task("icons", function() {
	return gulp.src(path.join(paths.entypo, "**/*.svg")).pipe(flatten()).pipe(
			rename(function(path) {
				// rename 'resize-100%.svg' because the maven-scm-publish-plugin
				// has
				// problems with the percent sign which it considers an escape
				// character
				if (path.basename === "resize-100%") {
					path.basename = "resize-100"
				}
			})).pipe(replace(/id="[^"]+"/, 'id="icon"')) // set id of all
	// icons to a fixed
	// value so we can
	// reference it
	// easier
...
gulp.task("raml", function(done) {
	...
	var buildSource = paths.raml_outputdir + "/spec.raml";
	mkdirp(paths.raml_outputdir, function(err) {
		if (err) {
			console.log(err);
			process.exit(1);
		} else {
			...
			function() {
				...
				build(function() {
					gutil.log("Finished", "'" + gutil.colors.cyan("rebuilding")
							+ "'", "after", gutil.colors
							.magenta(prettyHrtime(process.hrtime(start))));
				}, true);
			});
});
