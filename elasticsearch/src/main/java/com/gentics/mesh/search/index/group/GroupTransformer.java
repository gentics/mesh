...
public class GroupTransformer extends AbstractTransformer<HibGroup> {
	...
	@Override
	public String generateVersion(HibGroup group, DataHolderContext dhc) {
		// No need to add users since the creator/editor edge affects the group version
		return ETag.hash(group.getElementVersion());
	}

	private JsonObject toDocument(HibGroup group, DataHolderContext dhc, boolean withVersion) {
		JsonObject document = new JsonObject();
		document.put(NAME_KEY, group.getName());
		addBasicReferences(document, group);
		addPermissionInfo(document, group, "group", dhc);
		if (withVersion) {
			document.put(MappingHelper.VERSION_KEY, generateVersion(group, dhc));
	...
	@Override
	public JsonObject toDocument(HibGroup group, DataHolderContext dhc) {
		return toDocument(group, dhc, true);
	}
