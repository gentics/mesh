...
import com.gentics.mesh.graphdb.query.MeshArcadeGraphEdgeQuery;
...
/**
 * Implementation of an ArcadeDB transaction.
 * 
 ...
 * </ul>
 */
public class ArcadeDBTx extends AbstractTx<ArcadeGraph, DelegatingFramedArcadeGraph> {

	private static final Logger log = LoggerFactory.getLogger(ArcadeDBTx.class);
	...
	@Inject
	public ArcadeDBTx(GraphDBMeshOptions options, Database db, GraphDBBootstrapInitializer boot,
			GraphDBDaoCollection daos, Lazy<CacheCollection> caches, SecurityUtils security, ArcadeStorage provider,
		...
		if (activeTx != null) {
			...
			isWrapped = true;
			init((DelegatingFramedArcadeGraph) activeTx.getGraph());
		} else {
			DelegatingFramedArcadeGraph transaction = new DelegatingFramedArcadeGraph((ArcadeGraph) provider.rawTx(), typeResolver);
			init(transaction);
		}
		this.txData = txData;
	...
	@Override
	protected void init(DelegatingFramedArcadeGraph transactionalGraph) {
		Mesh mesh = boot.mesh();
		if (mesh != null) {
	...
	public <E extends Element> E getElement(Object id) {
		return (E) currentGraph.getBaseGraph().getDatabase().lookupByRID((RID) id, true).asDocument();
	}

	@Override
	public DelegatingFramedArcadeGraph getGraph() {
		return currentGraph;
	}

	@Override
	public MeshGraphVertexQuery vertexQuery(Class<?> vertexClass) {
		return new MeshArcadeGraphVertexQuery(currentGraph.getBaseGraph(), vertexClass);
	}

	@Override
	public MeshGraphEdgeQuery edgeQuery(Class<?> vertexClass, String edgeLabel) {
		return new MeshArcadeGraphEdgeQuery(currentGraph.getBaseGraph(), vertexClass, edgeLabel);
	}
}
