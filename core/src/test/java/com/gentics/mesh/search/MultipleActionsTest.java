...
public class MultipleActionsTest extends AbstractNodeSearchEndpointTest {
	...
	public void MultipleActionsTest() throws Exception {
		...
		Completable actions = getNodesBySchema(SCHEMA_NAME)
			.compose(flatMapCompletable(this::deleteNode)).toCompletable()
			.andThen(deleteSchemaByName(SCHEMA_NAME))
			.andThen(createTestSchema()).doOnSuccess(newSchema::set).toCompletable()
			.andThen(rootNodeReference$)
			.andThen(Observable.range(1, nodeCount))
	...
	private Single<SchemaResponse> getSchemaByName(String schemaName) throws JSONException {
		return client().searchSchemas(getSimpleTermQuery("name.raw", schemaName)).toObservable()
				.flatMapIterable(it -> it.getData()).toSingle();
	}
	...
	private Single<SchemaResponse> createTestSchema() {
		...
		SchemaCreateRequest request = new SchemaCreateRequest()
			.setName(SCHEMA_NAME)
			.addField(new StringFieldSchemaImpl().setName("testfield1"))
			.addField(new StringFieldSchemaImpl().setName("testfield2"));
