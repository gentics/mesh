...
public class StopWatchLogger {
	...
	public void log(String name, double time) {
		if (log.isDebugEnabled()) {
			log.debug("Logging {" + name + "} with time:" + df.format(time));
		}
	...
	public void flush() {
		...
		for (Entry<String, Double> entry : entries.entrySet()) {
			example.getTestcases().add(new Testcase(entry.getKey(), clazz.getName(), String.valueOf(entry.getValue())));
		}
