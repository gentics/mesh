...
public class LinkRendererVerticleTest extends AbstractRestVerticleTest {
	...
	/**
	 * Test rendering JSON object with links in attribute values.
	 * Links use single quotes, double quotes and no quotes
	 */
	@Test
	public void testLinkInJson() {
		...
		JsonObject jsonObject = new JsonObject()
				.put("quotes", "prefix {{mesh.link('" + newsNode.getUuid() + "')}} postfix")
				.put("doublequotes", "prefix {{mesh.link(\"" + newsNode.getUuid() + "\")}} postfix")
				.put("noquotes", "prefix {{mesh.link(" + newsNode.getUuid() + ")}} postfix");

		JsonObject expected = new JsonObject()
				.put("quotes", "prefix /api/v1/dummy/webroot/News/News+Overview.en.html postfix")
