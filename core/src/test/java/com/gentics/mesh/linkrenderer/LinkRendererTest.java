...
public class LinkRendererTest extends AbstractIsolatedBasicDBTest {
	...
	public void testLinkReplacerTypeOff() {
		try (NoTrx noTx = db.noTrx()) {
			...
			String replacedContent = replacer.replace(project().getLatestRelease().getUuid(), GraphFieldContainerEdge.Type.DRAFT, content, LinkType.OFF,
					null, null);
	...
	public void testLinkReplacerTypeShort() {
		try (NoTrx noTx = db.noTrx()) {
			...
			String uuid = newsNode.getUuid();
			final String content = "{{mesh.link('" + uuid + "')}}";
			String replacedContent = replacer.replace(project().getLatestRelease().getUuid(), GraphFieldContainerEdge.Type.DRAFT, content, LinkType.SHORT,
					null, null);
	...
	public void testLinkReplacerTypeMedium() {
		try (NoTrx noTx = db.noTrx()) {
			...
			String uuid = newsNode.getUuid();
			final String content = "{{mesh.link('" + uuid + "')}}";
			String replacedContent = replacer.replace(project().getLatestRelease().getUuid(), GraphFieldContainerEdge.Type.DRAFT, content,
					LinkType.MEDIUM, null, null);
	...
	public void testLinkReplacerTypeFull() {
		try (NoTrx noTx = db.noTrx()) {
			...
			String uuid = newsNode.getUuid();
			final String content = "{{mesh.link('" + uuid + "')}}";
			String replacedContent = replacer.replace(project().getLatestRelease().getUuid(), GraphFieldContainerEdge.Type.DRAFT, content, LinkType.FULL,
					null, null);
	...
	public void testBinaryFieldLinkResolving() {
		try (NoTrx noTx = db.noTrx()) {
			...
			String replacedContent = replacer.replace(project().getLatestRelease().getUuid(), GraphFieldContainerEdge.Type.DRAFT, meshLink, LinkType.FULL,
					null, null);
