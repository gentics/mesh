...
public class SchemaChangeTest extends AbstractBasicDBTest {
	...
	public void testDomainModel() {
		...
		SchemaChange change = Database.getThreadLocalGraph().addFramedVertex(AbstractSchemaChange.class);
	...
	public void testChangeChain() {
		...
		for (int i = 0; i < 3; i++) {
			...
			if (oldChange == null) {
				oldChange = change;
				assertNull("The change has not yet been connected to any schema", oldChange.getFromSchemaContainer());
				...
				assertNotNull("The change has been connected to the schema container and thus the connection should be loadable",
						oldChange.getFromSchemaContainer());
			} else {
		...
		assertNull(oldChange.getToSchemaContainer());
		oldChange.setToSchemaContainer(containerB);
		assertNotNull(oldChange.getToSchemaContainer());
		...
		assertEquals("The last change should be connected to the containerB but it was not.", containerB.getUuid(),
				lastChange.getToSchemaContainer().getUuid());
		assertNull("The change has no from schema container because it it part of a chain of changes.", lastChange.getFromSchemaContainer());
