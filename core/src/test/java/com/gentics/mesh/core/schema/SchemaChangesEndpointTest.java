...
public class SchemaChangesEndpointTest extends AbstractNodeSearchEndpointTest {
	...
	public void testAddRestrictedField() throws Exception {
		...
		try (Tx tx = tx()) {
			...
			assertArrayEquals(new String[] {"5678"}, boot().contentDao().getGraphFieldContainer(node, "en").getSchemaContainerVersion().getSchema().getField("newField", StringFieldSchema.class).getAllowedValues());
		}
	}
	...
	public void testRemoveFieldRestriction() throws Exception {
		...
		try (Tx tx = tx()) {
			...
			assertTrue("The version of the original schema and the schema that is now linked to the node should be different.",
				!Objects.equals(currentVersion.getVersion(), boot().contentDao().getGraphFieldContainer(node, "en").getSchemaContainerVersion().getVersion()));
			assertArrayEquals(new String[] {}, boot().contentDao().getGraphFieldContainer(node, "en").getSchemaContainerVersion().getSchema().getField("newField", StringFieldSchema.class).getAllowedValues());
		}
	}
