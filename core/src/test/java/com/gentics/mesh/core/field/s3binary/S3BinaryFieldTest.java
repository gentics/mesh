...
public class S3BinaryFieldTest extends AbstractFieldTest<S3BinaryFieldSchema> {
    ...
    public void testFieldUpdate() {
        try (Tx tx = tx()) {
            ...
            S3Binary s3binary = mesh().s3binaries().create("1234", "1234/s3", "img.jpg").runInExistingTx(tx);
            S3BinaryGraphField field = container.createS3Binary(S3_BINARY_FIELD, s3binary);
            ...
            S3Binary loadedS3Binary = loadedField.getS3Binary();
    ...
    public void testClone() {
        try (Tx tx = tx()) {
            ...
            S3Binary s3binary = mesh().s3binaries().create("1234","1234/s3","img.jg").runInExistingTx(tx);
    ...
    public void testEqualsNull() {
        try (Tx tx = tx()) {
            NodeGraphFieldContainerImpl container = tx.getGraph().addFramedVertex(NodeGraphFieldContainerImpl.class);
            S3Binary s3binary = mesh().s3binaries().create(UUIDUtil.randomUUID(), "/s3", "img.jpg").runInExistingTx(tx);
    ...
    public void testEqualsRestField() {
        try (Tx tx = tx()) {
            ...
            S3Binary s3binary = mesh().s3binaries().create("1234", container.getUuid() + "/s3", "img.jpg").runInExistingTx(tx);
            S3BinaryGraphField fieldA = container.createS3Binary("fieldA", s3binary);
