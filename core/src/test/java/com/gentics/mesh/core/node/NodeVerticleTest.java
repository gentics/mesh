...
public class NodeVerticleTest extends AbstractBasicCrudVerticleTest {
	...
	public void testMoveDuplicateWebrootPath() {
		...
		call(() -> getClient().moveNode(PROJECT_NAME, uuid, otherParent.getUuid()), CONFLICT,
				"node_conflicting_segmentfield_move", "filename", conflictingName);
	}
	...
	public void testDuplicateWithPublished() {
		...
		db.noTrx(() -> {
			call(() -> getClient().publishNode(PROJECT_NAME, otherNodeUuid), CONFLICT,
					"node_conflicting_segmentfield_publish", "filename", conflictingName);
	...
	public void testTakeNodeOffline() {
		...
		db.noTrx(() -> {
			for (String language : Arrays.asList("en", "de")) {
				for (String property : Arrays.asList(NodeGraphFieldContainerImpl.WEBROOT_PROPERTY_KEY,
					...
					assertThat(folder("products").getGraphFieldContainer(language).getImpl().getProperty(property,
							String.class)).as("Property " + property + " for " + language).isNotNull();
				}
			}
			return null;
		});
		...
		db.noTrx(() -> {
			for (String language : Arrays.asList("en", "de")) {
				...
				assertThat(folder("products").getGraphFieldContainer(language).getImpl().getProperty(property,
						String.class)).as("Property " + property + " for " + language).isNull();
