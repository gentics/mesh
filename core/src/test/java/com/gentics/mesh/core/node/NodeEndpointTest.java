...
public class NodeEndpointTest extends AbstractMeshTest implements BasicRestTestcases {
	...
	public void testCreateNodeWithBogusLanguageCode() throws GenericRestException, Exception {
		...
		String folderUuid = db().tx(() -> folder("news").getUuid());
	...
	public void testCreateNodeInBaseNode() {
		String schemaUuid = db().tx(() -> schemaContainer("content").getUuid());
		String folderUuid = db().tx(() -> project().getBaseNode().getUuid());
	...
	public void testCreateFolder() {
		String schemaUuid = db().tx(() -> schemaContainer("folder").getUuid());
		String folderUuid = db().tx(() -> folder("news").getUuid());
	...
	public void testCreateForLatestRelease() {
		...
		try (Tx tx = tx()) {
			...
			for (ContainerType type : Arrays.asList(ContainerType.INITIAL, ContainerType.DRAFT)) {
				assertThat(newNode.getGraphFieldContainer("en", releaseUuid(), type)).as(type + " Field container for initial release").isNull();
		...
		try (Tx tx = tx()) {
			...
			for (int page = 1; page <= totalPages; page++) {
				MeshResponse<NodeListResponse> pageFuture = client()
						.findNodes(PROJECT_NAME, new PagingParametersImpl(page, perPage), new VersioningParametersImpl().draft()).invoke();
				latchFor(pageFuture);
	...
	public void testReadPublishedNodesNoPermission() {
		...
		while (!nodes.isEmpty()) {
			...
			List<NodeResponse> publishedNodes = nodes.stream().map(node -> {
				String uuid = db().tx(() -> node.getUuid());
						...
						updateFuture.setHandler(uh -> {
							if (uh.failed()) {
								...
								MeshResponse<NodeResponse> readFuture = client()
										.findNodeByUuid(PROJECT_NAME, uh.result().getUuid(), new VersioningParametersImpl().draft()).invoke();
	...
	public void testUpdateMultithreaded() throws InterruptedException {
		...
		assertEquals("2015", db().tx(() -> folder("2015").getLatestDraftFieldContainer(english()).getString("slug").getString()));
		VersionNumber version = db().tx(() -> folder("2015").getLatestDraftFieldContainer(english()).getVersion());
		...
		for (int i = 0; i < nJobs; i++) {
			...
			// VersionNumber currentVersion = db().tx(() -> folder("2015").getLatestDraftFieldContainer(english()).getVersion());
	...
	public void testReadVersionByNumber() {
		try (Tx tx = tx()) {
			...
			assertThat(call(() -> client().findNodeByUuid(PROJECT_NAME, uuid, new NodeParametersImpl().setLanguages("de"),
					new VersioningParametersImpl().setVersion("0.1")))).as("German version 0.1").hasVersion("0.1").hasLanguage("de")
							.hasStringField("name", "eins");
			assertThat(call(() -> client().findNodeByUuid(PROJECT_NAME, uuid, new NodeParametersImpl().setLanguages("de"),
					new VersioningParametersImpl().setVersion("0.2")))).as("German version 0.2").hasVersion("0.2").hasLanguage("de")
							.hasStringField("name", "zwei");
			assertThat(call(() -> client().findNodeByUuid(PROJECT_NAME, uuid, new NodeParametersImpl().setLanguages("de"),
					new VersioningParametersImpl().setVersion("0.3")))).as("German version 0.3").hasVersion("0.3").hasLanguage("de")
	...
	public void testReadNodeForRelease() {
		...
		try (Tx tx = tx()) {
			...
			assertThat(
					call(() -> client().findNodeByUuid(PROJECT_NAME, uuid, new VersioningParametersImpl().setRelease(release().getName()).draft())))
	...
	public void testReadNodeVersionForRelease() {
		...
		try (Tx tx = tx()) {
			...
			updateRequest.setVersion(new VersionReference(null, "1.0"));
			response = call(
					() -> client().updateNode(PROJECT_NAME, uuid, updateRequest, new VersioningParametersImpl().setRelease(release().getName())));
			...
			assertThat(call(() -> client().findNodeByUuid(PROJECT_NAME, uuid,
					new VersioningParametersImpl().setRelease(release().getName()).setVersion("0.1")))).as("Initial Release Version")
							.hasVersion("0.1").hasStringField("name", "2015");
			assertThat(call(() -> client().findNodeByUuid(PROJECT_NAME, uuid,
					new VersioningParametersImpl().setRelease(release().getName()).setVersion("0.1")))).as("New Release Version").hasVersion("0.1")
							.hasStringField("name", "2015 v0.1 new release");
			assertThat(call(() -> client().findNodeByUuid(PROJECT_NAME, uuid,
					new VersioningParametersImpl().setRelease(release().getName()).setVersion("1.1")))).as("Initial Release Version")
							.hasVersion("1.1").hasStringField("name", "2015 v1.1 initial release");
			assertThat(call(() -> client().findNodeByUuid(PROJECT_NAME, uuid,
					new VersioningParametersImpl().setRelease(release().getName()).setVersion("0.2")))).as("New Release Version").hasVersion("0.2")
	...
	public void testUpdate() throws GenericRestException, Exception {
		...
		final Node node = db().tx(() -> content("concorde"));
		NodeGraphFieldContainer origContainer = db().tx(() -> {
	...
	public void testUpdateCreateLanguage() {
		...
		final Node node = db().tx(() -> folder("2015"));
		final String uuid = db().tx(() -> folder("2015").getUuid());
