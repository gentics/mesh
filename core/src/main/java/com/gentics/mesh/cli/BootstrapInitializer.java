...
public class BootstrapInitializer {
	...
	public void initMandatoryData() throws JsonParseException, JsonMappingException, IOException, MeshSchemaException {
		...
		try (Trx tx = db.trx()) {
			...
			Group adminGroup = groupRoot.findByName("admin").toBlocking().single();
			if (adminGroup == null) {
			...
			adminRole = roleRoot.findByName("admin").toBlocking().single();
			if (adminRole == null) {
	...
	public void initPermissions() {
		try (Trx tx = db.trx()) {
			Role adminRole = meshRoot().getRoleRoot().findByName("admin").toBlocking().single();
	...
	protected void initLanguages(LanguageRoot root) throws JsonParseException, JsonMappingException, IOException {
		...
		for (Map.Entry<String, LanguageEntry> entry : languageSet.entrySet()) {
			...
			Language language = meshRoot().getLanguageRoot().findByName(languageName).toBlocking().single();
			if (language == null) {
