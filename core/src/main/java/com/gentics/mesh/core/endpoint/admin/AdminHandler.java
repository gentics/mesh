...
public abstract class AdminHandler extends AbstractHandler {
	...
	public void handleOpenAPIv3(InternalActionContext ac) {
		handleOpenAPIv3(ac, ac.getOpenAPIParameters().getFormat(), ac.getOpenAPIParameters().getVersion());
	}
	 ...
	 */
	public void handleOpenAPIv3(InternalActionContext ac, com.gentics.mesh.etc.config.Format format2, com.gentics.mesh.etc.config.Version version) {
		...
		try {
			ac.send(generator.generate(
					Stream.of(
							...
							//... from project plugin root
							options.getOpenAPIOptions().isExcludePlugins() ? Stream.<Pair<Router, String>>empty() : routerStorageRegistry.getInstances().stream().map(rr -> Pair.of(rr.root().apiRouter().projectsRouter().projectRouter().pluginRouter().getRouter(), "/api/v" + MeshVersion.CURRENT_API_VERSION + "/{project}/plugins/")),
