...
public class S3BinaryMetadataExtractionHandlerImpl extends AbstractHandler {
	...
	@Inject
	public S3BinaryMetadataExtractionHandlerImpl(Lazy<BootstrapInitializer> boot, Database db, S3BinaryStorage s3BinaryStorage, HandlerUtilities utils,
	...
	public void handleMetadataExtraction(InternalActionContext ac, String nodeUuid, String fieldName) {
		...
		s3BinaryStorage
			...
			if (nonNull(fileBuffer) && fileBuffer.getBytes().length > 0) {
				...
				.subscribe(model ->
				ac.send(model, HttpResponseStatus.OK), ac::fail);
	}
