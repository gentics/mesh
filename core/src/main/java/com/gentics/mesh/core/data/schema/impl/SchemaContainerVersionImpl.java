...
public class SchemaContainerVersionImpl extends
	...
	@Override
	public Iterator<? extends NodeGraphFieldContainer> getDraftFieldContainers(String branchUuid, int limit) {
		OrientDBDatabase database = (OrientDBDatabase) mesh().database();
		Iterator<? extends NodeGraphFieldContainer> iterator;
		String uuid = getUuid();
		if (limit < 1) {
			iterator = toStream(database.getVertices(
	...
	public TraversalResult<? extends Node> getNodes(String branchUuid, User user, ContainerType type) {
		return new TraversalResult<>(getSchemaContainer().getNodes().stream()
			.filter(node -> GraphFieldContainerEdgeImpl.matchesBranchAndType(node.getId(), branchUuid, type)
				&& user.hasPermissionForId(node.getId(), type == PUBLISHED ? READ_PUBLISHED_PERM : READ_PERM)));
	}
