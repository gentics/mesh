 ...
 * @see BranchMigration
 */
// TODO this should be rewritten DB-agnostic
@Singleton
	...
	private void migrateNode(HibNode node, EventQueueBatch batch, HibBranch oldBranch, HibBranch newBranch, List<Exception> errorsDetected) {
		try {
			db.tx((tx) -> {
				...
				Result<? extends HibNodeFieldContainer> drafts = contentDao.getGraphFieldContainers(node, oldBranch, DRAFT);
				Result<? extends HibNodeFieldContainer> published = contentDao.getGraphFieldContainers(node, oldBranch, PUBLISHED);
