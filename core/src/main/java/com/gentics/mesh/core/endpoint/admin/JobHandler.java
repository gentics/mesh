...
public class JobHandler extends AbstractCrudHandler<HibJob, JobResponse> {
	...
	public void handleReadList(InternalActionContext ac) {
		utils.syncTx(ac, tx -> {
			...
			Page<? extends HibJob> page = boot.jobDao().findAll(ac, pagingInfo);
	...
	public void handleDelete(InternalActionContext ac, String uuid) {
		...
		try (WriteLock lock = writeLock.lock(ac)) {
			utils.syncTx(ac, () -> {
				...
				HibJob job = dao.loadObjectByUuidNoPerm(uuid, true);
				db.tx(() -> {
					if (job.hasFailed()) {
						dao.delete(job, new DummyBulkActionContext());
					} else {
	...
	public void handleResetJob(InternalActionContext ac, String uuid) {
		utils.syncTx(ac, tx -> {
			...
			return dao.transformToRestSync(job, ac, 0);
		}, (model) -> ac.send(model, OK));
	}
