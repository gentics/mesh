...
public class DockerClientFactory {
	...
	private static final List<DockerClientProviderStrategy> CONFIGURATION_STRATEGIES = asList(
			new EnvironmentAndSystemPropertyClientProviderStrategy(), new UnixSocketClientProviderStrategy(),
			new ProxiedUnixSocketClientProviderStrategy(), new DockerMachineClientProviderStrategy(), new WindowsClientProviderStrategy());
	...
	public DockerClient client() {
		...
		if (!preconditionsChecked) {
			...
			log.info("Connected to docker: \n" + "  Server Version: " + dockerInfo.getServerVersion() + "\n" + "  API Version: " + activeApiVersion
					...
					+ dockerInfo.getMemTotal() / (1024 * 1024) + " MB");
			...
			if (!TestcontainersConfiguration.getInstance().isDisableChecks()) {
				...
				runInsideDocker(client,
						cmd -> cmd.withCmd("/bin/sh", "-c", "while true; do printf 'hello' | nc -l -p 80; done")
								.withBinds(new Bind(mountableFile.getResolvedPath(), new Volume("/dummy"), AccessMode.ro))
								.withExposedPorts(new ExposedPort(80)).withPublishAllPorts(true),
						(dockerClient, id) -> {
	...
	private void checkDiskSpace(DockerClient dockerClient, String id) {
		...
		try {
			dockerClient.execStartCmd(dockerClient.execCreateCmd(id).withAttachStdout(true).withCmd("df", "-P").exec().getId())
					.exec(new ExecStartResultCallback(outputStream, null)).awaitCompletion();
		} catch (Exception e) {
	...
	/**
	 * @param providerStrategyClass
	 *            a class that extends {@link DockerMachineClientProviderStrategy}
