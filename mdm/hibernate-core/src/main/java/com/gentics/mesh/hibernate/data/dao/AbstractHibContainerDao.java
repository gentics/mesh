...
public abstract class AbstractHibContainerDao<
			...
			D extends SC, 
			DV extends SCV
		> extends AbstractHibDaoGlobal<SC, R, D> implements PersistingContainerDao<R, RM, RE, SC, SCV, M>, RootDao<Project, SC> {

	protected final DaoHelper<SCV, DV> versionDaoHelper;
	private final RootDaoHelper<SC, D, Project, HibProjectImpl> rootDaoHelper;

	public AbstractHibContainerDao(RootDaoHelper<SC, D, Project, HibProjectImpl> rootDaoHelper, HibPermissionRoots permissionRoots,
	...
	@Override
	public SchemaChange<?> createPersistedChange(SCV version, SchemaChangeOperation schemaChangeOperation) {
	...
	@Override
	public Result<SCV> findActiveSchemaVersions(Branch branch) {
		EntityManager em = currentTransaction.getEntityManager();
	...
	@Override
	public boolean contains(Project project, SC schema) {
		return rootDaoHelper.findByElementInRoot(project, "dbUuid", ((HibDatabaseElement) schema).getDbUuid()).findAny().isPresent();
	}

	@Override
	public long globalCount(Project root) {
		return rootDaoHelper.countInRoot(root);
	}

	@Override
	public Stream<? extends SC> findAllStream(Project root, InternalActionContext ac,
	...
	@Override
	public Page<? extends SC> findAll(Project root, InternalActionContext ac, PagingParameters pagingInfo) {
	...
	@Override
	public Page<? extends SC> findAllNoPerm(Project root, InternalActionContext ac,
			PagingParameters pagingInfo) {
		return rootDaoHelper.findAllInRoot(root, ac, pagingInfo, Optional.empty(), null, false);
	}

	@Override
	public SC findByName(Project root, String name) {
		return firstOrNull(rootDaoHelper.findByElementInRoot(root, null, "name", name, null));
	}

	@Override
	public SC findByName(Project root, InternalActionContext ac, String name, InternalPermission perm) {
		return firstOrNull(rootDaoHelper.findByElementInRoot(root, ac, "name", name, perm));
	}

	@Override
	public SC findByUuid(Project root, String uuid) {
	...
	@Override
	public Result<? extends Branch> getBranches(SCV version) {
		EntityManager em = em();
		return new TraversalResult<>(em.createQuery(
	 ...
	 * @return
	 */
	public SC findByUuid(Project root, UUID uuid) {
