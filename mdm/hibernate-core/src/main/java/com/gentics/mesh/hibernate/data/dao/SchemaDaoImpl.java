...
public class SchemaDaoImpl 
    ...
    public void deleteVersions(Set<Pair<String, String>> versions) {
		...
		""").setParameter("versionUuids", versionUuids).executeUpdate());
		{
			...
			List<UUID[]> list = em().createQuery("select pv.dbUuid, v.dbUuid, nv.dbUuid from schemaversion v left join v.previousVersion pv left join v.nextVersion nv where v.dbUuid in :versionUuids", UUID[].class).setParameter("versionUuids", versionUuids).getResultList();
