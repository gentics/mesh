...
public interface FieldTypeChange extends SchemaFieldChange {
	...
	private BooleanFieldModel changeToBoolean(FieldSchemaContainer oldSchema, FieldContainerModel oldContent) {
		String fieldName = getFieldName();
		FieldSchema fieldSchema = oldSchema.getField(fieldName);

		FieldModel oldField = oldContent.getFields().getField(fieldName, fieldSchema);
		if (oldField == null) {
			return null;
		}
	...
	private NumberFieldModel changeToNumber(FieldSchemaContainer oldSchema, FieldContainerModel oldContent) {
		String fieldName = getFieldName();
		FieldSchema fieldSchema = oldSchema.getField(fieldName);
	...
	private DateFieldModel changeToDate(FieldSchemaContainer oldSchema, FieldContainerModel oldContent) {
		String fieldName = getFieldName();
		FieldSchema fieldSchema = oldSchema.getField(fieldName);
	...
	private HtmlFieldModel changeToHtml(FieldSchemaContainer oldSchema, FieldContainerModel oldContent) {
		String fieldName = getFieldName();
		FieldSchema fieldSchema = oldSchema.getField(fieldName);
		...
		FieldModel field = oldContent.getFields().getField(fieldName, fieldSchema);
		if (field == null) {
			return null;
		}
	...
	private StringFieldModel changeToString(FieldSchemaContainer oldSchema, FieldContainerModel oldContent) {
		String fieldName = getFieldName();
		FieldSchema fieldSchema = oldSchema.getField(fieldName);
	...
	private BinaryFieldModel changeToBinary(FieldSchemaContainer oldSchema, FieldContainerModel oldContent) {
		String fieldName = getFieldName();
		FieldSchema fieldSchema = oldSchema.getField(fieldName);
	...
	private S3BinaryFieldModel changeToS3Binary(FieldSchemaContainer oldSchema, FieldContainerModel oldContent) {
		String fieldName = getFieldName();
		FieldSchema fieldSchema = oldSchema.getField(fieldName);
	...
	private FieldListModel changeToList(FieldSchemaContainer oldSchema, FieldContainerModel oldContent) {
		String fieldName = getFieldName();
		FieldSchema fieldSchema = oldSchema.getField(fieldName);
	...
	private <T> FieldListModel<T> nullableList(T[] input, Supplier<FieldListModel<T>> output) {
		if (input == null) {
			return null;
		} else {
			FieldListModel<T> list = output.get();
			list.setItems(Arrays.asList(input));
			return list;
		}
	}

	private MicronodeFieldModel changeToMicronode(FieldSchemaContainer oldSchema, FieldContainerModel oldContent) {
		String fieldName = getFieldName();
		FieldSchema fieldSchema = oldSchema.getField(fieldName);
	...
	private NodeFieldModel changeToNode(FieldSchemaContainer oldSchema, FieldContainerModel oldContent) {
		String fieldName = getFieldName();
		FieldSchema fieldSchema = oldSchema.getField(fieldName);
