 ...
 * versions.
 */
public interface SchemaVersion extends FieldSchemaVersionElement<SchemaResponse, SchemaVersionModel, SchemaReference, Schema, SchemaVersion> {
	 ...
	 */
	default boolean usesMicroschema(Microschema microschema) {
	 ...
	 * @return hash
	 */
	default String getMicroschemaVersionHash(Branch branch, Map<String, String> replacementMap) {
		...
		if (microschemaNames.isEmpty()) {
			...
			for (BranchMicroschemaVersion edge : branch.findAllLatestMicroschemaVersionEdges()) {
	 ...
	 */
	default Set<String> getFieldsUsingMicroschema(Microschema microschema) {
