...
public interface MicronodeFieldList extends MicroschemaListableField, ReferencingListField<MicronodeField, MicronodeFieldListModel, Micronode> {
	 ...
	 * @return
	 */
	default Micronode createMicronode(MicroschemaVersion microschemaContainerVersion) {
	 ...
	 * @return
	 */
	Micronode createMicronodeAt(Optional<Integer> maybeIndex, MicroschemaVersion microschemaContainerVersion);

	@Override
	default MicronodeFieldListModel transformToRest(InternalActionContext ac, String fieldKey, List<String> languageTags, int level) {

		MicronodeFieldListModel restModel = new MicronodeFieldListImpl();

		for (MicronodeField item : getList()) {
	 ...
	 * @return
	 */
	default Single<Boolean> update(InternalActionContext ac, MicronodeFieldListModel list) {
		...
		return Observable.<Boolean>create(subscriber -> {
			...
			Iterator<MicronodeFieldModel> it = list.getItems().stream().map(item -> {
				if (item == null) {
			...
			Observable.fromIterable(() -> it).flatMap(item -> {
				...
				MicroschemaVersion container = microschemaDao.fromReference(tx.getProject(ac), microschemaReference, tx.getBranch(ac));
	...
	@Override
	default List<Micronode> getValues() {
		return getList().stream().map(MicronodeField::getMicronode).collect(Collectors.toList());
	}

	@Override
	default boolean listEquals(Object obj) {
		if (obj instanceof MicronodeFieldList) {
			MicronodeFieldList graphList = ((MicronodeFieldList) obj);
		...
		if (obj instanceof MicronodeFieldListImpl) {
			...
			List<MicronodeFieldModel> restList = restField.getItems();

			List<? extends MicronodeField> graphList = getList();
		...
		return ReferencingListField.super.listEquals(obj);
	}
}
