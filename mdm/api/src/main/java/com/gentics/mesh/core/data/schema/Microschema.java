...
public interface Microschema
	extends FieldSchemaElement<MicroschemaResponse, MicroschemaVersionModel, MicroschemaReference, Microschema, MicroschemaVersion>, BucketableElement {
	 ...
	 * @return
	 */
	MicroschemaVersion getLatestVersion();
	 ...
	 * @param version
	 */
	void setLatestVersion(MicroschemaVersion version);
	 ...
	 */
	@Override
	default Map<Branch, MicroschemaVersion> findReferencedBranches() {
		Map<Branch, MicroschemaVersion> references = new HashMap<>();
		MicroschemaDao microschemaDao = Tx.get().microschemaDao();
		for (MicroschemaVersion version : microschemaDao.findAllVersions(this)) {
