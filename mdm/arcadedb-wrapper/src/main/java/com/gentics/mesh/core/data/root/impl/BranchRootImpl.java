...
public class BranchRootImpl extends AbstractRootVertex<Branch> implements BranchRoot {
	...
	public Branch findByName(String name) {
		ElementFrame element = db().index().checkIndexUniqueness(Branch.UNIQUENAME_INDEX_NAME, BranchImpl.class, getUniqueNameKey(name));
		if (element != null) {
			return GraphDBTx.getGraphTx().getGraph().frameElementExplicit(element.element(), BranchImpl.class);
		} else {
			return null;
		}
	}
