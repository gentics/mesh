...
public class BranchDaoWrapperImpl extends AbstractRootDaoWrapper<BranchResponse, HibBranch, Branch, HibProject>
	...
	@Inject
	public BranchDaoWrapperImpl(Lazy<GraphDBBootstrapInitializer> boot, Lazy<GraphDatabase> db) {
		super(boot);
		this.db = db;
	}
	...
	public HibBranch findConflictingBranch(HibBranch branch, String name) {
		...
		ElementFrame element = db.get().index().checkIndexUniqueness(Branch.UNIQUENAME_INDEX_NAME, graphBranch,
				graphBranch.getRoot().getUniqueNameKey(name));
		if (element == null) {
			return null;
		} else {
			return graph.frameElementExplicit(db.get().index().checkIndexUniqueness(Branch.UNIQUENAME_INDEX_NAME, graphBranch,
				graphBranch.getRoot().getUniqueNameKey(name)).element(), BranchImpl.class);
		}
	}
