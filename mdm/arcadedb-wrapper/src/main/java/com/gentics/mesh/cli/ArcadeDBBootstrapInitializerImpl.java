...
@Singleton
public class ArcadeDBBootstrapInitializerImpl extends AbstractBootstrapInitializer implements GraphDBBootstrapInitializer {

	private static Logger log = LoggerFactory.getLogger(ArcadeDBBootstrapInitializerImpl.class);

	protected final ChangelogSystem changelogSystem;

	protected ArcadeDBDatabase db;
	...
	@Inject
	public ArcadeDBBootstrapInitializerImpl(ChangelogSystem changelogSystem, ArcadeDBDatabase db, ServerSchemaStorageImpl schemaStorage,
	...
	protected void initCluster(MeshOptions options, PostProcessFlags flags, boolean isInitMode) throws Exception {
		...
		if (isInitMode) {
			...
			// We need to init the graph db before starting the ArcadeDB Server. Otherwise the database will not get picked up by the arcadedb server which
			...
			// Start ArcadeDB Server which will open the previously created db and init hazelcast
