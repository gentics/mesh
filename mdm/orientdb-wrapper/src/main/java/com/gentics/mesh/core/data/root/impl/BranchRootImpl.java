...
public class BranchRootImpl extends AbstractRootVertex<Branch> implements BranchRoot {
	...
	private Branch create(String name, HibUser creator, String uuid, boolean setLatest, HibBranch baseBranch, boolean assignSchemas,
		...
		if (assignSchemas) {
			assignSchemas(mesh().boot().branchDao(), creator, baseBranch, branch, false, batch);
		}
	...
	public Branch create(InternalActionContext ac, EventQueueBatch batch, String uuid) {
		...
		assignSchemas(mesh().boot().branchDao(), creator, baseBranch, branch, true, batch);
	 ...
	 * @param batch
	 */
	private void assignSchemas(BranchDao branchDao, HibUser creator, HibBranch baseBranch, HibBranch newBranch, boolean migrate, EventQueueBatch batch) {
		...
		if (baseBranch != null && migrate) {
			for (HibSchemaVersion schemaVersion : baseBranch.findActiveSchemaVersions()) {
				branchDao.assignSchemaVersion(newBranch, creator, schemaVersion, batch);
			}
		}

		// assign the newest schema versions of all project schemas to the branch
		for (HibSchema schemaContainer : getProject().getSchemaContainerRoot().findAll()) {
			branchDao.assignSchemaVersion(newBranch, newBranch.getCreator(), schemaContainer.getLatestVersion(), batch);
		}

		// ... same for microschemas
		if (baseBranch != null && migrate) {
			for (HibMicroschemaVersion microschemaVersion : baseBranch.findActiveMicroschemaVersions()) {
				branchDao.assignMicroschemaVersion(newBranch, creator, microschemaVersion, batch);
			}
		}

		for (HibMicroschema microschema : getProject().getMicroschemaContainerRoot().findAll()) {
			branchDao.assignMicroschemaVersion(newBranch, newBranch.getCreator(), microschema.getLatestVersion(), batch);
		}
	}
