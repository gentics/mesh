...
public class NodeMigrationJobImpl extends JobImpl {
	...
	private NodeMigrationActionContextImpl prepareContext() {
		...
		try {
			return db().tx(tx -> {
				...
				tx.createBatch().add(createEvent(SCHEMA_MIGRATION_START, STARTING)).dispatch();
			...
			return handler.migrateNodes(context)
				.doOnComplete(() -> {
					...
					db.tx(tx -> {
						...
						tx.createBatch().add(createEvent(SCHEMA_MIGRATION_FINISHED, FAILED)).dispatch();
					});
				});
	...
	private void finalizeMigration(Database db, NodeMigrationActionContext context) {
		...
		db.tx(tx -> {
			tx.createBatch().add(createEvent(SCHEMA_MIGRATION_FINISHED, COMPLETED)).dispatch();
		});
	}
