...
icon:check[] Auth: A `HttpOnly` flag is set onto the JWT auth cookie. A `Secure` flag is set onto the JWT auth cookie, if a HTTPS-only server is started in Mesh.
