...
icon:check[] Server: The Mesh docker container has a new healthcheck now, which uses a JMX MBean. The healthcheck will run the command `java -cp mesh.jar com.gentics.mesh.monitor.jmx.JMXProbe` in the container and will succeed as soon as the Mesh process is running. The health check will fail if an OutOfMemoryError was caught or if one
of the plugins (permanently) failed to initialize.
