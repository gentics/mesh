...
icon:plus[] Auth: Added JWT options to configure token content (`issuer` - `"iss"`, `audience` - `"aud"`) and token verification (`leeway` and `ignoreExpiration`).
...
icon:check[] Core: permission for creating API tokens are now checked only when creating a token for a different user than the requesting user.
