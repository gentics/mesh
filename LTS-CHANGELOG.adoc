...
icon:check[] Search: Search requests or graphQl requests containing a search, which should wait until the search (indexing) is idle, will not wait for ever, but will
only wait until either the indexing becomes idle or a timeout occurs. The timeout can be configured with the setting `search.waitForIdleTimeout`, which defaults to 60_000 ms.
Additionally, some metrics have been added for monitoring of indexing and search. See link:{{< relref "monitoring.asciidoc" >}#metrics[Monitoring Metrics] for details.
