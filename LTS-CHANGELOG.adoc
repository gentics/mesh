...
icon:check[] Core: Added support for looking up the node field content by webroot path and a field name.

icon:check[] Rest: Added new endpoint for webroot path field lookup.
