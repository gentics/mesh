...
icon:plus[] Clustering: The `cluster.topologyChangeReadOnly` setting and `MESH_CLUSTER_TOPOLOGY_CHANGE_READONLY` enrsync curl iputils busybox-extras tar nanovironment setting have been added. By enabling this flag, Mesh will be automatically set in `readOnly` mode,
