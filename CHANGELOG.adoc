...
icon:plus[] Core: A support of database level filtering/sorting has been added. The GraphQL Mesh options has a new preference `nativeQueryFiltering` with options: `OFF` (old filtering), `ALWAYS` (DB filtering is forced), 
...
icon:plus[] OrientDB: Basic experimental support for DB filtering/sorting is added.

icon:plus[] GraphQL: A simplified, native filtering friendly version of `regex` string operation, `like`, has been added. The syntax is analogous to SQL operator `LIKE`, accepting wildcards `%` as a zeroable set of characters, and `_` as a non-zeroable character.
