...
icon:plus[] Core: It is now possible to disable extracting of metadata or content from binaries for metadata on a per-field basis. See link:{{< relref "building-blocks.asciidoc" >}}#_schema_field_types[schema field types documentation] for more information.
