...
icon:plus[] OrientDB 3.0.x: The `orientdb-server-config.xml`, `hazelcast.xml`, `default-distributed-db-config.json` files need to be updated. You can remove those and restart the server to get the correct default configuration files.
...
icon:plus[] Core: The way language references are stored has been refactored to increase performance and reduce contention during database operations. A changelog entry will be invoked which migrated the data.

icon:check[] Clustering: The OrientDB 3.0.13 update adresses various issues which were related to clustering.

icon:check[] Migrations: The schema migration code has been optimized for speed and memory usage.

icon:check[] Migrations: Schemas migrations will now always be executed on the cluster node which also handled the REST request. Previously the migration was executed on a random node in the cluster.
