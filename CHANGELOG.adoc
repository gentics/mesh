...
icon:plus[] Core: A support of database level filtering/sorting has been added. The GraphQL Mesh options has a new preference `nativeQueryFiltering` with options: `NEVER` (old filtering), `ALWAYS` (DB filtering is forced), 
...
icon:plus[] GraphQL: Added new filters for field types: list, micronode.
