...
icon:check[] Core: It was possible to upload binaries with empty filenames. This has been fixed now: it is enfored that
				  a binary upload has a filename and content type which are not empty.
...
icon:check[] Elasticsearch: Search requests failed on systems with many schemas. This has been fixed now.

icon:plus[] Demo: The AngularJS version of the link:https://demo.getmesh.io/demo[demo application] was updated.
...
icon:check[] Core: When the migration for multiple nodes faild during a schema migration the error details could become very long.
...
icon:check[] Elasticsearch: Fixed handling of `search.url` settings which contained https URLs.

icon:check[] REST: The `/api/v1/auth/logout` endpoint will now correctly delete the `mesh.token` cookie.
