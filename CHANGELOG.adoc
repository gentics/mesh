...
[[Next release]]
== Next release (insert date here)

icon:check[] Image Manipulation: Added new `smart` and `prop` resizing options that crop and/or resize an image to the desired dimensions instead of distorting it when providing width _and_ height parameters that result in an aspect ratio that is different from the original images aspect ratio.
The old behaviour can now be achieved by using `resize=force`.

CAUTION: New default image resize mode is `smart` which potentially crops an image when requested aspect ratio that diverge from the source images aspectratio. To avoid cropping, request either only width _or_ height or make sure that the aspect ratio of the requested dimensions fit the source images aspect ratio. Alternatively request the image with `resize=prop` or `resize=force`. For details on image manipulation and resizing opions visit link:https://getmesh.io/docs/image-manipulation/ .
[quote]
____
Should you have used resizing to intentionally distort images you now have to change your Image Manipulator calls to include the `resize=force` option.
____
CAUTION: The different resizing of images caused by this change might not apply when an image is already in the cache. To make sure that the changes take effect, the cache in `data/binaryImageCache/` must be cleared.
