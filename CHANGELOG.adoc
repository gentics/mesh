...
icon:plus[] Core: The RAM consumption by OrientDB is now configurable via settings `storage.memoryPreallocate` (should the whole RAM amount, allowed for cache usage, be preallocated at once on Mesh start, default: true) and `storage.memoryLeftToHost` (how much of OS/container RAM should OrientDB leave available to the host, default: 256m for container environment, 2g for bare OS).
