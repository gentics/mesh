...
icon:plus[] It is now possible to execute elasticsearch queries within the graphql query.

icon:plus[] It is now possible to resolve a partial web root path using the `child` field of a node.
...
icon:plus[] It is now possible to resolve information about the running mesh instance via graphql.
