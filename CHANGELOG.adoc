...
icon:plus[] Docker: The Gentics Mesh server docker container has been updated:
  ...
  * Additional group permissions will be set to the application folders to allow root 
  group to access and write into those.
  ...
  * The embedded Elasticsearch folder has been removed from the server image. The embedded ES will automatically be extracted to the `/mesh/elasticsearch` folder when embedded mode is enabled.
