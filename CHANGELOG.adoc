...
icon:plus[] GraphQL: It is now possible to retrieve the path for a content using the `path` field.

icon:plus[] GraphQL: Contents can now directly be queried via the `contents` field.

icon:plus[] Core: It is now possible to issue API keys via the `GET /api/v1/users/:userUuid/apiKey` endpoint. API keys do not expire and work in conjunction with the regular JWT authentication mechanism. These keys should only be used when SSL is enabled. The `DELETE /api/v1/users/:userUuid/apiKey` endpoint can be used to revoke the issued API. Only one API key is supported for one user. Generating a new API key will invalidate the previously issued key.
