...
icon:plus[] The `/api/v1/search/sync` endpoint can now be used to trigger the differential synchronization of search indices. 
            The indices will no longer be dropped and repopulated. Instead only needed actions will be executed to sync the index with the Gentics Mesh data.
...
icon:plus[] GraphQL: It is now possible to link:https://getmesh.io/docs/beta/graphql.html#_filtering[filter lists of nodes].
