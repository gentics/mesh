...
icon:plus[] Search: It is now possible to use encrypted and authenticated connections between Gentics Mesh and Elasticsearch. New settings and environment variables have been added. https://github.com/gentics/mesh/issues/759[#759]
...
The `username` and `password` will only be used when specified. These settings can be used to configure basic auth for Elasticsearch.

Please note that SSL support for Elasticsearch connections was already part of Gentics Mesh. The newly added `certPath` and `caPath` settings can be used to configure self-signed certificates which may be used for Elasticsearch in-transit encryption.
Both paths only accept certificates in the *PEM* format. You don't need to configure the cert paths if you are using a already trusted certificate.
