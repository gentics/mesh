...
icon:check[] Backup: Fixed a bug in the backup handler which prevented the backup process to switch back to the correct status after a backup failed due to missing filesystem permissions.
...
icon:plus[] REST: The authentication and element loading performance has been increased.
