...
CAUTION: GraphQL: Various issues with the permission handling in GraphQL have been adressed. Please note that this change may affect your GraphQL query results. It is thus advised to check whether your query returns the data you need with the current set of active permissions.

icon:plus[] GraphQL: Permission errors will now also contain information about the location of the error within the query.

icon:plus[] GraphQL: The `nodeType` argument has been added to all locations where it is possible to load nodes. This argument can be used to select whether `draft` or `published` nodes should be returned.
