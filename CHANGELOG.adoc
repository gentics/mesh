...
icon:check[] Image Manipulation: Add new smart resizing option that crops an image to the desired dimensions instead of distorting it when providing width _and_ height paramenters that result in an aspect ratio that is different from the original images aspect ratio.
The he old behaviour can now be achieved by using `resize=force`.

CAUTION: New default image resize mode is `smart` which crops an image instead of distorting it
[quote]
____
