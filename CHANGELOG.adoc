...
icon:plus[] The *Webroot-Response-Type* header can now be used to differntiate between a webroot binary and webroot node response. The values of this header can either be *binary* or *node*. 
...
icon:plus[] REST: Added new endpoints `/api/v1/admin/jobs` to list and check queued migration jobs. The new endpoints are described in the link:/docs/beta/features.html#_executing_migrations[feature documentation].
