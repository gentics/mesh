...
icon:bullhorn[] REST: The `availableLanguages` field now also contains the publish information of the languages of a node.
...
icon:plus[] Config: It is now possible to configure the host to which the Gentics Mesh http server should bind to. Default is still 0.0.0.0 (all interfaces).

icon:plus[] REST: The `/api/v1/admin/reindex` and `/api/v1/admin/createMappings` have been changed from `GET` to `POST`.
...
icon:plus[] Manual Change: Configuration changes. For already existing `mesh.yml` files, the `nodeName` setting has to be added. Choose any name for the mesh instance.
