...
icon:plus[] REST: It is now possible to specify a path prefix for branches. When specified all resolved paths will include the prefix. The webroot endpoint will also utilize the prefix to resolve the nodes. The prefix can be set for new projects or for update requests on existing branches.
