...
icon:bullhorn[] Image Manipulation: A new smart resizing option was added.

[quote]
____
It crops an image to the desired dimensions instead of distorting it when providing `width` _and_ `height` paramenters which would result in an aspect ratio that is different from the original images aspect ratio. The he old behaviour can now be achieved by using the `resize=force` parameter.

The different resizing of images caused by this change might not apply when an image is already in the cache. To make sure that the changes take effect, the cache in `data/binaryImageCache/` must be manually cleared.
____
