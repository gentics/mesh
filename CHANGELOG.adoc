...
icon:bullhorn[] Image Manipulation: The new resizing options `smart` and `prop` have been added.
...
To avoid cropping, request either only `width` _or_ `height` or make sure that the aspect ratio of the requested dimensions fit the source images aspect ratio. Alternatively request the image with `?resize=prop` or `?resize=force`. For details on image manipulation and resizing opions read the link:{{< relref "plugin-types/image-manipulation.asciidoc" >}}[Image Manipulation documentation].
