...
icon:check[] Core: When updating a schema/microschema, omitted properties, or properties that are set to null, will now assume the value of the previous version instead of the default value. This prevents unwanted node migrations.
