...
CAUTION: Java Rest Client: The default field values for `NodeResponse` and `AbstractGenericRestResponse` has been updated. 
...
Applications which use affected models will most likly not needed to updated since the returned response will always set the affected fields.

icon:plus[] REST: The `?fields` query parameter which can be used to filter response properties will now work more granular on node responses.

CAUTION: Custom node migration scripts have been removed. The script system was a source of possible errors and performance problems. Also, the nashorn javascript engine has been deprecated with the release of Java 11. If you need to change data for a migration, use the REST Api instead.
...
icon:check[] Backup: Fixed `NullPointerException` which occured when the backup directory was missing and the backup endpoint was invoked. link:https://github.com/gentics/mesh/issues/463[#463]
