...
The Gentics Mesh server will now by default bind to port `8081` in order to provide the new monitoring API. New configuration settings have been added to allow configuration of this server.
...
icon:plus[] REST: The `/users` endpoint response will now contain all roles assigned to the loaded users.

icon:plus[] GraphQL: The `roles` field is now available for user fields.
