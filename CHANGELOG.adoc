...
icon:plus[] GraphQL: The nested `content` and `contents` fields have been removed. The properties of those fields have been merged with the node field.
...
icon:plus[] GraphQL: A language can now be specified when loading a node reference fields using the `lang` argument.
