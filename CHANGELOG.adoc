...
link:{{< relref "features.asciidoc" >}}#controlling-versioning[Controlling Versioning]

Versioning will now be **disabled** by default. You can change this default setting within the `mesh.yml` file. 
Schema's now may contain a `versioned` flag to control versioning on a per schema basis.
