...
The `security.oauth2` configuration section was removed. Instead the `security.publicKeys` field was added.

In order to provide support for more authentication providers it is now possible to directly specify the public keys in the configuration which will be used to validate the access tokens.
...
It is now also possible for Authentication Service Plugins to provide Json Web Keys (JWK) to Gentics Mesh. Plugins can load the accepted JWK's from the authentication provider server and return it to Gentics Mesh.
____
