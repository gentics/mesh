...
icon:plus[] REST: The `GET /api/v1/search/status` endpoint response has been updated.
...
icon:check[] REST: Fixed an error which lead to inconsistent properties being shown in the job endpoint response.

icon:check[] Search: When calling reindex via the `POST /api/v1/search/reindex` reindexing stopped after a certain amount of
