...
public class WebRootFieldEndpointETagTest extends AbstractMeshTest {
	...
	public void testResizeImage() throws IOException {
		...
		String etag = callETag(() -> client().webrootField(PROJECT_NAME, "binary", path, params, new VersioningParametersImpl().setVersion("draft")));
		callETag(() -> client().webrootField(PROJECT_NAME, "binary", path, params, new VersioningParametersImpl().setVersion("draft")), etag, false, 304);

		params.setHeight(103);
		String newETag = callETag(() -> client().webrootField(PROJECT_NAME, "binary", path, params, new VersioningParametersImpl().setVersion("draft")), etag,
			false, 200);
		callETag(() -> client().webrootField(PROJECT_NAME, "binary", path, params, new VersioningParametersImpl().setVersion("draft")), newETag, false, 304);
	...
	public void testReadBinaryNode() throws IOException {
		...
		String etag = callETag(() -> client().webrootField(PROJECT_NAME, "binary", path, new VersioningParametersImpl().draft(),
			new NodeParametersImpl().setResolveLinks(LinkType.FULL)));
		assertNotNull(etag);

		// Check whether 304 is returned for correct etag
		assertEquals(etag, callETag(() -> client().webrootField(PROJECT_NAME, "binary", path, new VersioningParametersImpl().draft(),
	...
	public void testReadOne() {
		...
		try (Tx tx = tx()) {
			...
			request.getFields().put("reference", new NodeFieldImpl().setUuid(folder("2015").getUuid()));
			call(() -> client().updateNode(projectName(), node.getUuid(), request));
			tx.success();
		}

		String responseTag = callETag(
			() -> client().webrootField(PROJECT_NAME, "content", path, new VersioningParametersImpl().draft(), new NodeParametersImpl().setLanguages("en", "de")));
		...
		try (Tx tx = tx()) {
			...
			// Check whether 304 is returned for correct etag
			assertEquals(etag, callETag(() -> client().webrootField(PROJECT_NAME, "content", path, new VersioningParametersImpl().draft(),
