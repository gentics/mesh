...
public class GroupEndpointTest extends AbstractMeshTest implements BasicRestTestcases {
	...
	public void testCreateWithUuid() throws Exception {
		...
		try (Tx tx = tx()) {
			assertThat(restGroup).matches(request);
			Group reloadedGroup = tx.groupDao().findByUuid(uuid);
	...
	public void testBatchCreation() {
		try (Tx tx = tx()) {
			RoleDao roleDao = tx.roleDao();
			BaseElement root = tx.data().permissionRoots().group();
	...
	public void testCreateReadDelete() throws Exception {
		try (Tx tx = tx()) {
			...
			Group foundGroup = tx.groupDao().findByUuid(restGroup.getUuid());
	...
	public void testReadPermittedSorted() throws Exception {
		for (int i = 0; i < 10; i++) {
			...
			GroupCreateRequest request = new GroupCreateRequest();
			request.setName(name);
			GroupResponse response = call(() -> client().createGroup(request));
			if ((i % 2) == 0) {
				tx(tx -> {
					tx.roleDao().revokePermissions(role(), tx.groupDao().findByUuid(response.getUuid()), READ_PERM);
				});
			}
		}
		GroupListResponse list = call(() -> client().findGroups(new SortingParametersImpl("name", SortOrder.DESCENDING)));
	...
	public void testUpdate() throws GenericRestException, Exception {
		...
		try (Tx tx = tx()) {
			assertThat(restGroup).matches(request);
			Group reloadedGroup = tx.groupDao().findByUuid(groupUuid);
			assertEquals("The group should have been updated", name, reloadedGroup.getName());
		}
	}
