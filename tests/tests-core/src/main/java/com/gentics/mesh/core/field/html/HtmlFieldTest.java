...
public class HtmlFieldTest extends AbstractFieldTest<HtmlFieldSchema> {
	...
	public void testFieldUpdate() {
		try (Tx tx = tx()) {
			...
			HtmlField htmlField = container.createHTML(HTML_FIELD);
			...
			// Test field loading
			HtmlField reloadedHTMLField = container.getHtml(HTML_FIELD);
	...
	public void testFieldTransformation() throws Exception {
		try (Tx tx = tx()) {
			...
			HtmlField field = container.createHTML(HTML_FIELD);
		...
		try (Tx tx = tx()) {
			...
			com.gentics.mesh.core.rest.node.field.HtmlFieldModel deserializedNodeField = response.getFields().getHtmlField("htmlField");
	...
	public void testEquals() {
		try (Tx tx = tx()) {
			...
			HtmlField fieldA = container.createHTML("fieldA");
			HtmlField fieldB = container.createHTML("fieldB");
	...
	public void testEqualsNull() {
		try (Tx tx = tx()) {
			NodeFieldContainer container = CoreTestUtils.createContainer(createFieldSchema(true));
			HtmlField fieldA = container.createHTML("htmlField1");
			assertFalse(fieldA.equals((FieldModel) null));
			assertFalse(fieldA.equals((HtmlField) null));
		}
	}
	...
	public void testEqualsRestField() {
		try (Tx tx = tx()) {
			NodeFieldContainer container = CoreTestUtils.createContainer(createFieldSchema(true));
			HtmlField fieldA = container.createHTML(HTML_FIELD);
	...
	public void testUpdateFromRestValidSimpleValue() {
		try (Tx tx = tx()) {
			...
			invokeUpdateFromRestValidSimpleValueTestcase(HTML_FIELD, FILLTEXT, (container) -> {
				...
				HtmlField field = container.getHtml(HTML_FIELD);
