...
public class JWTPayloadValidationTest extends AbstractMeshTest {
	...
	public void backupOptions() {
		if (this.originalOptions == null) {
			this.originalOptions = JsonUtil.readValue(JsonUtil.toJson(this.options()), this.options().getClass());
		}
	}
	...
	public void testOptionChanges() throws IOException {
		try (Tx tx = tx()) {
			MeshOptions clientOptions = JsonUtil.readValue(JsonUtil.toJson(this.originalOptions), this.options().getClass());
