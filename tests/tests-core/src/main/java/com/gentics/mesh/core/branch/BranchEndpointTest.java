...
public class BranchEndpointTest extends AbstractMeshTest implements BasicRestTestcases {
	...
	public void testReadByBogusUUID() throws Exception {
		...
		call(() -> client().findBranchByUuid(PROJECT_NAME, bogusUuid), NOT_FOUND, "object_not_found_for_uuid", bogusUuid);
	}
	...
	public void testReadPermittedSortedWrongField() throws Exception {
		MeshRestClientMessageException error = call(() -> client().findBranches(PROJECT_NAME, new SortingParametersImpl("existiertleidernicht", SortOrder.DESCENDING)), BAD_REQUEST);
