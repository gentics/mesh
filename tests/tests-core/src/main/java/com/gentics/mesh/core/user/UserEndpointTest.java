...
public class UserEndpointTest extends AbstractMeshTest implements BasicRestTestcases {
	...
	public void testUpdateUsingExpiredToken() {
		...
		tx(tx -> {
			User user1 = tx.userDao().findByUuid(uuid);
	...
	public void testUpdateUsingToken() {
		...
		User updatedUser = tx(tx -> { return tx.userDao().findByUuid(uuid); });
	...
	public void testUpdate() throws Exception {
		...
		try (Tx tx = tx()) {
			...
			User reloadedUser = tx.userDao().findByUsername(newName);
	...
	public void testUpdatePasswordWithEmptyPassword() {
		...
		try (Tx tx = tx()) {
			User reloadedUser = tx.userDao().findByUuid(uuid);
	...
	public void testUpdatePassword() throws JsonGenerationException, JsonMappingException, IOException, Exception {
		...
		try (Tx tx = tx()) {
			User reloadedUser = tx.userDao().findByUsername(username);
			assertNotEquals("The hash should be different and thus the password updated.", oldHash, reloadedUser.getPasswordHash());
		}
	}
	...
	public void testUpdatePasswordWithNoPermission() throws JsonGenerationException, JsonMappingException, IOException, Exception {
		...
		try (Tx tx = tx()) {
			User reloadedUser = tx.userDao().findByUuid(userUuid());
	...
	public void testCreate() throws Exception {
		...
		try (Tx tx2 = tx()) {
			...
			User user = tx2.userDao().findByUuid(restUser.getUuid());
			assertThat(restUser).matches(user);
		}
	}
	...
	public void testDeleteByUUID() throws Exception {
		try (Tx tx = tx()) {
			...
			try (Tx tx2 = tx()) {
				User loadedUser = tx2.userDao().findByUuid(uuid);
	...
	public void testDeleteByUUIDWithNoPermission() throws Exception {
		...
		try (Tx tx = tx()) {
			...
			uuid = user.getUuid();
			assertNotNull(uuid);
			Role role = tx(() -> tx.roleDao().findByUuid(role().getUuid()));
	...
	public void testDeleteByUUID2() throws Exception {
		...
		try (Tx tx = tx()) {
			RoleDao roleDao = tx.roleDao();
			UserDao userDao = tx.userDao();
			Role role = roleDao.findByUuid(role().getUuid());
	...
	public void testLocationHeader() {
		...
		try (Tx tx = tx()) {
			User user = tx.userDao().findByUsername(name);
