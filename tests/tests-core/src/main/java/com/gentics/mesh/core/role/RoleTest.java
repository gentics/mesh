...
public class RoleTest extends AbstractMeshTest implements BasicObjectTestcases {
	...
	public void testCreate() throws Exception {
		try (Tx tx = tx()) {
			...
			Role role = tx.roleDao().findByUuid(uuid);
			assertNotNull(role);
	...
	public void testGrantMultiplePermission() {
		...
		try (Tx tx = tx()) {
			...
			Role anonymous = roleDao.findByName("anonymous");
	...
	public void testGetMultiplePermissions() {
		try (Tx tx = tx()) {
			...
			assertThat(testRole).as("Test role").isNotNull();
			Node node = folder("news");
			Map<Role, Set<InternalPermission>> result = roleDao.getPermissions(new HashSet<>(Arrays.asList(admin, anonymous, testRole)), node);
	...
	public void testGetMultiplePermissionsForNoRoles() {
		try (Tx tx = tx()) {
			RoleDao roleDao = tx.roleDao();
			Node node = folder("news");
			Map<Role, Set<InternalPermission>> result = roleDao.getPermissions(Collections.emptySet(), node);
			assertThat(result).as("Permissions result")
				.isEmpty();
		}
	}
	...
	public void testRolesOfGroup() throws InvalidArgumentException {
		...
		try (Tx tx = tx()) {
			...
			Page<? extends Role> roles = groupDao.getRoles(group(), requestUser, new PagingParametersImpl(1, 10L));
	...
	public void testFindAllVisible() throws InvalidArgumentException {
		...
		try (Tx tx = tx()) {
			RoutingContext rc = mockRoutingContext();
			InternalActionContext ac = new InternalRoutingActionContextImpl(rc);
			Page<? extends Role> page = tx.roleDao().findAll(ac, new PagingParametersImpl(1, 5L));
	...
	public void testFindByUUID() {
		try (Tx tx = tx()) {
			Role role = tx.roleDao().findByUuid(role().getUuid());
			assertNotNull(role);
