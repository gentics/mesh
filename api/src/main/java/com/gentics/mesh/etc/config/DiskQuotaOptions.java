...
public class DiskQuotaOptions implements Option {
	...
	public long getAbsoluteWarnThreshold(File storageDir) {
		if (warnThresholdPercent < 0 || warnThresholdAbsolute < 0) {
			Matcher percentageMatcher = ConfigUtils.QUOTA_PATTERN_PERCENTAGE.matcher(this.warnThreshold);
			Matcher absoluteMatcher = ConfigUtils.QUOTA_PATTERN_SIZE.matcher(this.warnThreshold);
			...
			if (percentageMatcher.matches()) {
				...
				this.warnThresholdAbsolute = ConfigUtils.getBytes(absoluteMatcher);
			} else {
				this.warnThresholdPercent = 0;
				this.warnThresholdAbsolute = 0;
			}
	...
	public long getAbsoluteReadOnlyThreshold(File storageDir) {
		if (readOnlyThresholdPercent < 0 || readOnlyThresholdAbsolute < 0) {
			Matcher percentageMatcher = ConfigUtils.QUOTA_PATTERN_PERCENTAGE.matcher(this.readOnlyThreshold);
			Matcher absoluteMatcher = ConfigUtils.QUOTA_PATTERN_SIZE.matcher(this.readOnlyThreshold);
			...
			if (percentageMatcher.matches()) {
				...
				this.readOnlyThresholdAbsolute = ConfigUtils.getBytes(absoluteMatcher);
			} else {
				this.readOnlyThresholdPercent = 0;
				this.warnThresholdAbsolute = 0;
			}
	...
	public void validate(MeshOptions options) {
		if (checkInterval > 0) {
			...
			if (!ConfigUtils.QUOTA_PATTERN_SIZE.matcher(readOnlyThreshold).matches()
					&& !ConfigUtils.QUOTA_PATTERN_PERCENTAGE.matcher(readOnlyThreshold).matches()) {
			...
			if (!ConfigUtils.QUOTA_PATTERN_SIZE.matcher(warnThreshold).matches()
					&& !ConfigUtils.QUOTA_PATTERN_PERCENTAGE.matcher(warnThreshold).matches()) {
